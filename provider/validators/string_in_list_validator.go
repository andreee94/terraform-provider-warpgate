...
func (v StringIn) Description(ctx context.Context) string {
	return fmt.Sprintf("String must be one of %v.", v.ValidValues)
}

// MarkdownDescription returns a markdown formatted description of the validator's behavior, suitable for a practitioner to understand its impact.
func (v StringIn) MarkdownDescription(ctx context.Context) string {
	return fmt.Sprintf("String must be one of `%v`.", v.ValidValues)
}

// Validate runs the main validation logic of the validator, reading configuration data out of `req` and updating `resp` with diagnostics.
func (v StringIn) Validate(ctx context.Context, req tfsdk.ValidateAttributeRequest, resp *tfsdk.ValidateAttributeResponse) {
